...
<body>
    <div class="container">
        <div class="header">
            <h3 class=" text-center" style="letter-spacing: 3px; word-spacing: 7px; text-transform:capitalize;">
                <?= __d('attendance', 'Generate Routine For Admit Card') ?>
            </h3>
        </div>
    ...
    <?php if (isset($scms_admit_cards)) { ?>
        <div style="background-color: #f2f2f2; padding: 10px; margin-top: 50px;">
            ...
            <table class="table table-bordered table-striped">
                ...
                <tbody>
                    <?php foreach ($scms_admit_cards as $key => $course) { ?>
                        <tr>
                            <td>
                                <input type="text" name="subject<?php echo $key; ?>" class="form-control" value="<?php echo htmlspecialchars($course['course_name']); ?>">
                            </td>
                            <td>
                                <input type="datetime-local" name="time_from<?php echo $key; ?>" value="<?= date('Y-m-d\TH:i', strtotime($course['datetime_from'])); ?>" class="form-control">
                            </td>
                            <td>
                                <input type="datetime-local" name="time_to<?php echo $key; ?>" value="<?= date('Y-m-d\TH:i', strtotime($course['datetime_to'])); ?>" class="form-control">
                            </td>
                        </tr>
                    <?php } ?>
                </tbody>
            ...
            <input type="hidden" class="hidden" id="hidden" name="level_id" value="<?php echo htmlspecialchars($request_data['level_id']); ?>">
            <input type="hidden" class="hidden" id="hidden" name="term_cycle_id" value="<?php echo $request_data['term_cycle_id']; ?>">
    ...
    <?php } elseif (isset($scms_student_cycles)) { ?>
        <div style="background-color: #f2f2f2; padding: 10px; margin-top: 50px;">
                ...
                <tbody>
                    <?php foreach ($scms_student_cycles as $key => $course) { ?>
                        <tr>
                            ...
                            <td>
                                <input type="datetime-local" name="time_from<?php echo $key; ?>" value="<?= date("Y-m-d H:i:s", time() + 6 * 3600) ?>" class="form-control">
                            </td>
                            <td>
                                <input type="datetime-local" name="time_to<?php echo $key; ?>" value="<?= date("Y-m-d H:i:s", time() + 6 * 3600) ?>" class="form-control">
                            </td>
                        </tr>
                    <?php } ?>
                </tbody>
            ...
            <input type="hidden" class="hidden" id="hidden" name="level_id" value="<?php echo htmlspecialchars($request_data['level_id']); ?>">
            <input type="hidden" class="hidden" id="hidden" name="term_cycle_id" value="<?php echo $request_data['term_cycle_id']; ?>">
            <input type="hidden" class="hidden" id="hidden" name="session_id" value="<?php echo $request_data['session_id']; ?>">
            <div class="text-right mt-4 mb-4">
                <button type="submit" class="btn btn-info">SAVE</button>
            </div>
            <?php echo $this->Form->end(); ?>
