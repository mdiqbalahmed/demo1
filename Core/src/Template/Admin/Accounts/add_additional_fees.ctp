<?php $this->Form->unlockField('fees_title'); ?>
	...
	<section>
		...
		<div class="row mx-3 mt-2 p-3 form-box">
			<div class="col-md-4 col-12 mt-2">
				<label for="inputBR" class="form-label"><?= __d('accounts', 'Fees Title') ?> </label>
				<input name="fees_title" type="text" class="form-control" placeholder="Enter Additional Fees" required>
			</div>
			<div class="col-md-4 col-12 mt-2">
				<label for="inputBR" class="form-label"><?= __d('accounts', 'Value') ?> </label>
				<input name="value" type="number" step="any" min="0" class="form-control" placeholder="Initial Value">
			</div>
			<div class="col-md-4 col-12 mt-2">
				...
				<select id="inputState" class="form-select option-class" name="purpose_id" required>
					...
					<?php foreach ($options as $value => $option) { ?>
						<option value="<?= $option['purpose_id'] ?>"><?= h($option['purpose_name']) ?></option>
					<?php } ?>
				</select>
			</div>
	</section>
