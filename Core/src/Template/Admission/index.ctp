...
class ResultsController extends AppController
{
    ...
    /* public function index() {
        ...
        $selectedsession = Configure::read('Site.selectedSession');
        $decoded = json_decode($selectedsession, true);
        // echo '<pre>';
        // print_r($where);die;
        $scms_results = TableRegistry::getTableLocator()->get('scms_results');
        $results = $scms_results
                ...
                ->where(['scms_results.session_id IN' => $decoded])
                ->where($where)
                ->enableAutoFields(true)
                ->enableHydration(false)
                ->join([
    ...
    public function index()
    {
        ...
        if ($this->request->is(['post'])) {
            ...
            if (!empty($request_data['term_cycle_id'])) {
                $where['scms_results.term_cycle_id'] = $request_data['term_cycle_id'];
            }
        }
        ...
        // Ensure filtering only by $decoded if session_id is not provided
        if (empty($request_data['session_id'])) {
            $where['scms_results.session_id IN'] = $decoded;
        }
        // echo '<pre>';
        ...
        // $paginate = $this->paginate($results, ['limit' => $this->Paginate_limit])->toArray();
        $this->set('results', $results);
    }
    ...
    public function generateResult()
    {
        if ($this->request->is(['post'])) {
            ...
            foreach ($students as $key => $student) {
                ...
                //absent for marge subject
                $students[$key] = $this->merge_course_absent($students[$key]);
            ...
            $view = '/result/new_markSheet';
            if (
                in_array($request_data['level_id'], [6, 7, 8, 9]) &&
                in_array($request_data['session_id'], [7, 9])
            ) {
                $view = '/result/condition_markSheet';
            } elseif (in_array($request_data['session_id'], [7, 9])) {
    ...
    private function merge_course_absent($student)
    {
        foreach ($student['merge_course'] as $key => $merge_course) {
            if (isset($merge_course['total']['absent'])) {
                $student['merge_course'][$key]['result']['result'] = 'fail';
                $student['merge_course'][$key]['total']['result']['result'] = 'fail';
            }
        }
        return $student;
    }

    public function generateClasstestResult()
    {
        if ($this->request->is(['post'])) {
            $request_data = $this->request->getData();
            ...
            echo $this->render('/result/markSheet');
            exit;
        }
    ...
    private function get_merge_studnet_result($terms, $request_data, $template)
    {
        ...
        echo $this->render('/result/markSheet_merge');
        exit;
    }
    ...
    private function get_student_activity($student_term_cycle_ids, $value)
    {
        ...
        foreach ($student_activities as $activity) {
            ...
            $filter_student_activities[$activity['student_term_cycle_id']]['comment'][$activity['activity_id']] = $activity['comment'];
        }
        // echo '<pre>';
        // print_r($filter_student_activities);
        // die;
    ...
    private function update_and_save_result($result_where, $request_data, $students, $attendance_months_data, $template, $type)
    {
        ...
        $activityIds = $template['activity']['value'] ?? [];
        $activityIds = array_values(array_filter($activityIds, 'is_numeric'));

        if (!empty($activityIds)) {
            $this->update_result_activity($activityIds, $result_students_data['result_id']);
        }
    ...
    private function update_result_activity(array $activity_ids, $result_id)
    {
        $scmsResultActivity = TableRegistry::getTableLocator()->get('scms_result_activity');
        $scmsResultActivityRemark = TableRegistry::getTableLocator()->get('scms_result_activity_remark');
        $scmsActivity = TableRegistry::getTableLocator()->get('scms_activity');
        $scmsActivityRemark = TableRegistry::getTableLocator()->get('scms_activity_remark');

        $connection = $scmsResultActivity->getConnection();
        $connection->begin();

        try {

            /* ---------- DELETE PREVIOUS ---------- */
            $previous = $scmsResultActivity
                ->find()
                ->select(['result_activity_id'])
                ->where(['result_id' => $result_id])
                ->enableHydration(false)
                ->toArray();

            if (!empty($previous)) {
                $ids = array_column($previous, 'result_activity_id');

                $scmsResultActivityRemark->deleteAll([
                    'result_activity_id IN' => $ids
                ]);

                $scmsResultActivity->deleteAll([
                    'result_activity_id IN' => $ids
                ]);
            }
            ...
            foreach ($activity_ids as $activity_id) {
                ...
                // ✅ SAVE ENTITY (THIS GUARANTEES ID)
                $resultActivityEntity = $scmsResultActivity->newEntity([
                    'result_id'     => $result_id,
                    'activity_id'   => $activity->activity_id,
                    'activity_name' => $activity->name,
                ]);

                $savedActivity = $scmsResultActivity->save($resultActivityEntity);

                if (!$savedActivity) {
                    throw new \Exception('Failed to save result activity');
                }

                // ✅ THIS ID IS NEVER NULL
                $resultActivityId = $savedActivity->result_activity_id;

                /* ---------- INSERT REMARKS ---------- */
                $remarks = $scmsActivityRemark
                    ->find()
                    ->where([
                ...
                foreach ($remarks as $remark) {
                    ...
                    $remarkEntity = $scmsResultActivityRemark->newEntity([
                        'result_activity_id' => $resultActivityId,
                        'activity_remark_id' => $remark->activity_remark_id,
                        'remark_name'        => $remark->remark_name,
                    ]);

                    if (!$scmsResultActivityRemark->save($remarkEntity)) {
                        throw new \Exception('Failed to save activity remark');
                    }
                }
            }
    ...
    private function filter_data_for_view_single_course($students, $filter_heads, $template, $merit, $overall = false)
    {
        foreach ($students as $key => $student) {
            ...
            foreach ($student['courses'] as $courses) {
                ...
                $new_course['course_details']['total'] = $courses['parts'][1]['total_mark'] + $courses['parts'][2]['total_mark'] + $courses['parts'][3]['total_mark'] + $courses['parts'][5]['total_mark'];
        ...
        foreach ($students as $key => $student) {
            ...
            foreach ($student['merge_course'] as $merge_course_key => $merge_courses) {
                ...
                foreach ($merge_courses as $merge_course) {
                    ...
                    $new_merge_course_single['course_details']['course_id'] = $merge_course['course_id'];
                    $new_merge_course_single['course_details']['total'] = $merge_course['parts'][1]['total_mark'] + $merge_course['parts'][2]['total_mark'] + $merge_course['parts'][3]['total_mark'] + $merge_course['parts'][5]['total_mark'];
    ...
    public function resultMerit($id)
    {
        ...
        // $this->Flash->success('Merit Successfully Genarated', [
        //     'key' => 'positive',
        //     'params' => [],
        // ]);
        if ($type == 'single') {
    ...
    private function result_tabulation($students, $results, $section_id = false)
    {
        ...
        $scms_result_student_courses = $scms_result_student_course->find()->where(['result_student_id IN' => $result_student_ids])
            ->where(['scms_result_student_courses.course_id !=' => 0])
            ->enableAutoFields(true)
            ->enableHydration(false)
            ->order(['scms_courses.course_code' => 'ASC'])
            ->group('scms_courses.course_code')
            ->select([
        ...
        // echo '<pre>';
        // print_r($scms_result_student_courses);die;
        $term_course_cycle_part_type = TableRegistry::getTableLocator()->get('scms_term_course_cycle_part_type');
        $types = $term_course_cycle_part_type->find()->enableAutoFields(true)->enableHydration(false)->where(['term_course_cycle_part_type_id  !=' => '9999'])->toArray();
        foreach ($types as $type) {
            $type['value'] = '-';
    ...
    private function result_marksheet($students, $results)
    {
        ...
        //     // die;
        //     if (
        //         isset($student['student_term_cycle_id']) &&
        //         isset($studnet_activities[$student['student_term_cycle_id']])
        //     ) {

        //         $stcId = $student['student_term_cycle_id'];

        //         $students[$studentKey]['comments']
        //             = $studnet_activities[$stcId];
        //     }
        // }
        ...
        /* attendance */
        $month = array_fill(1, 13, ['count' => '--']);
        foreach ($result_attendance_month as $am) {
            $month[$am['month_id']]['count'] = $am['count'];
        }

        /* normalize students */
        $studentsByStc = [];
        $student_term_cycle_ids = [];
        $student_cycle_ids = [];
        ...
        foreach ($students as $student) {
            ...
            $studentsByStc[$stcId] = $student;
            $student_term_cycle_ids[] = $stcId;
            $student_cycle_ids[] = $student['student_cycle_id'];
        }

        $students = $studentsByStc;
        ...
        if (isset($template['activity'])) {
            ...
            $activities_return = $this->get_student_activity(
                $student_term_cycle_ids,
                $template['activity']['value']
            );

            $studentActivities = $activities_return['student_activities'] ?? [];

            foreach ($activities_return['scms_activity_remarks'] ?? [] as $i => $remark) {
                $scms_activity_remarks[$i] = $remark;
            }
        }

        /* attach comments */
        foreach ($students as $stcId => $student) {
            $students[$stcId]['comments'] = $studentActivities[$stcId] ?? [];
        }

        /* render */
        $this->layout = 'result';
        $this->set(compact('students', 'month', 'scms_activity_remarks'));
        ...
        if (
            in_array($results['level_id'], [6, 7, 8, 9])
            && in_array($results['session_id'], [7, 9])
        ) {
            $view = '/result/condition_markSheet';
        } elseif (in_array($results['session_id'], [7, 9])) {
            $view = '/result/markSheet';
        }
    ...
    public function get_scms_result_students($id, $where)
    {
        ...
        // echo '258';
        // echo '<pre>';
        // print_r($filter_result_studnets);die;
        return $filter_result_studnets;
    }
    ...
    private function result_merge_marksheet($merge_students, $request_data, $results)
    {
        ...
        // foreach ($merge_students as $key => $student) {
        //     $student = $this->get_guardians($student);
        //     $merge_students[$student['student_term_cycle_id']] = $student;
        //     $student_term_cycle_ids[] = $student['student_term_cycle_id'];
        //     $student_cycle_ids[] = $student['student_cycle_id'];
        // }

        $merge_students_fixed = [];

        foreach ($merge_students as $student) {
            $student = $this->get_guardians($student);

            $stcId = $student['student_term_cycle_id'];

            $merge_students_fixed[$stcId] = $student;
            $student_term_cycle_ids[] = $stcId;
            $student_cycle_ids[] = $student['student_cycle_id'];
        }

        $merge_students = $merge_students_fixed;
        ...
        if (isset($template['activity'])) {
            ...
            $activities_return = $this->get_student_activity(
                array_values(array_unique($student_term_cycle_ids)),
                $template['activity']['value']
            );
        ...
        // foreach ($view_students as $studentKey => $student) {

        //     if (
        //         isset($student['base_term']['student_term_cycle_id']) &&
        //         isset($studnet_activities[$student['base_term']['student_term_cycle_id']])
        //     ) {

        //         $stcId = $student['base_term']['student_term_cycle_id'];

        //         $view_students[$studentKey]['base_term']['comments']
        //             = $studnet_activities[$stcId];
        //     }
        // }

        foreach ($view_students as $studentKey => $student) {
            $stcId = $student['base_term']['student_term_cycle_id'] ?? null;

            if ($stcId && isset($studnet_activities[$stcId])) {
                $view_students[$studentKey]['base_term']['comments'] = $studnet_activities[$stcId];
            }
        }
    ...
    public function sendSMS($id)
    {
        ...
        $result_students = $scms_result_students->find()
            ->where(['scms_result_students.result_id' => $id])
            ->enableAutoFields(true)
            ->enableHydration(false)
            ->select([
                'level_id' => 'scms_student_cycle.level_id',
        ...
        $results = $scms_results
            ...
            ->select([
                ...
                'highest' => 'scms_result_highest.highest_mark'
            ])
            ...
            ->join([
                ...
                'scms_result_highest' => [
                    'table' => 'scms_result_highest',
                    'type' => 'LEFT',
                    'conditions' => ['scms_result_highest.result_id  = scms_results.result_id']
                ],
                'scms_levels' => [
                    'table' => 'scms_levels',
                    'type' => 'LEFT',
                    'conditions' => ['scms_result_highest.level_id = scms_levels.level_id'],
                ],
            ])->toArray();
        //         echo '<pre>';
        // print_r($results);die;
    ...
    public function resultPreview()
    {
        if ($this->request->is(['post'])) {
            ...
            if (count($nofilter_students) == 0) {
                ...
                return $this->redirect(['action' => 'resultPreview']);
            }
            $students = array();
            //array indexing in students depand on "student_term_cycle_id"
            foreach ($nofilter_students as $key => $student) {
            ...
            $this->render('/result/preview_marksheet');
        }
